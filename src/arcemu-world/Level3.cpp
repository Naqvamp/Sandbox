...
{	//Adding link support to the learn command. --Hemi
	if(spelllink== NULL)
		return 0;
...
bool ChatHandler::HandleLearnSpellCommand(const char* args, WorldSession *m_session)
{
	...
	if( spell == 0 )
	{	//Added link support to the learn command. --Hemi
		spell = GetSpellIDFromLink( args );
		if( spell == 0 )
		{	//If the argument is still 0 then no link or ID was provided. --Hemi
...
{	//Separating LearnAll from the Learn command. --Hemi
	Player *plr = getSelectedChar(m_session, true);
	if(!plr)
	{
		plr = m_session->GetPlayer();
...
{	//Command to learn all talents. --Hemi
	...
	sGMLog.writefromsession(m_session, "taught %s all talents.", plr->GetName());
	static uint32 talentarray[DRUID+1][9999] = {
		{ 0 }, 		// CLASS 0    ---
		{ 12664, 16466, 12659, 12697, 12962, 12666, 12963, 12296, 12867, 12714, 16494, 12785, 12292, 12704, 12815, 29889, 23695, 29725, 29859, 12294, 29838, 35451, 29623, 12818, 12677, 12753, 12727, 12764, 12975, 12945, 12800, 12789, 12811, 12807, 12765, 12803, 12809, 12958, 29600, 16542, 29595, 23922, 29792, 29146, 20243, 12838, 12856, 12879, 13002, 20496, 12323, 16492, 12861, 23588, 20503, 13048, 12330, 12328, 20505, 20501, 12974, 29592, 23881, 29776, 29763, 29801, 0 }, 	// CLASS 1  (Warrior)
		...
		{ 19556, 19587, 35030, 19551, 19612, 19575, 19560, 19596, 19620, 19573, 19602, 20895, 19577, 19592, 34454, 19625, 34460, 19574, 34465, 34470, 34692, 19415, 19431, 19425, 19420, 34954, 19458, 19434, 34949, 19468, 19490, 35103, 19503, 24691, 34476, 19511, 34484, 19506, 35111, 34489, 34490, 24295, 19153, 19500, 19160, 19388, 19300, 19233, 19245, 19259, 19263, 19377, 24283, 19287, 34496, 19373, 19306, 34493, 24297, 34499, 19386, 34503, 34839, 23989, 0 }, 	// CLASS 3  (Hunter)
		...
		{ 14791, 14528, 14787, 14767, 14769, 14774, 33172, 14751, 14777, 14771, 14783, 14772, 18555, 14752, 33182, 33190, 18550, 45244, 10060, 33205, 34912, 33206, 15012, 17191, 15011, 27904, 18535, 15237, 27816, 15363, 27790, 15014, 15017, 15018, 20711, 15031, 33154, 15356, 34860, 724, 33146, 33162, 34861, 15338, 15326, 15320, 15317, 15330, 15448, 15316, 15407, 15311, 17323, 15334, 15487, 15286, 27840, 33215, 33371, 15310, 15473, 33225, 33195, 34914, 0 }, 	// CLASS 5  (Priest)
		{ 0 }, 		// CLASS 6    ---
		{ 16112, 16108, 16130, 28998, 16161, 16164, 16116, 16120, 16544, 29065, 29180, 29000, 16089, 30668, 30674, 16582, 16166, 30671, 30681, 30706, 16229, 16217, 16209, 16240, 16225, 16198, 16234, 16189, 29191, 16208, 16221, 29202, 16188, 30866, 16213, 16190, 30886, 30869, 30873, 974, 17489, 16301, 16293, 16305, 16287, 16291, 16295, 43338, 16274, 16284, 16309, 29193, 16268, 29080, 30814, 29088, 30819, 30798, 17364, 30811, 30823, 0 }, 	// CLASS 7  (Shaman)
		{ 28332, 16766, 29440, 15053, 12497, 12475, 12571, 12953, 12472, 12488, 16758, 12519, 12985, 31669, 11958, 12490, 31672, 28595, 11426, 31678, 31686, 31687, 12341, 12360, 12848, 12353, 12342, 18460, 12350, 11366, 12351, 12873, 13043, 29076, 31640, 11368, 11113, 31642, 12400, 34296, 11129, 31680, 31660, 31661, 12592, 12842, 16770, 6085, 29447, 12577, 12606, 12469, 28574, 12605, 12598, 18464, 31570, 12043, 12503, 31575, 15060, 31573, 31583, 12042, 35581, 31588, 31589, 0 }, 	// CLASS 8  (Mage)
		{ 17803, 17782, 17792, 18123, 18127, 18129, 18134, 17877, 18136, 17918, 17930, 18073, 17836, 17959, 30302, 17958, 34939, 17962, 30296, 30292, 30283, 18178, 17814, 18180, 18372, 18183, 17805, 18829, 17787, 18288, 18219, 18095, 32383, 32394, 18265, 18223, 18275, 30064, 18220, 30057, 32484, 30108, 18693, 18696, 18701, 18704, 18707, 18744, 18756, 18708, 18750, 30145, 18710, 18773, 18822, 18788, 18768, 30328, 23825, 30321, 19028, 35693, 30248, 30146, 0 }, 	// CLASS 9  (Warlock)
		{ 0 }, 		// CLASS 10   ---
		{ 16938, 16862, 16949, 16941, 16931, 24866, 16979, 16944, 16968, 16975, 37117, 16999, 16857, 33873, 24894, 33856, 33957, 17007, 34300, 33869, 33917, 17055, 17061, 17073, 17068, 16835, 17108, 17122, 16864, 24972, 17113, 17116, 24946, 17124, 33880, 17078, 34153, 18562, 33883, 33890, 33891, 16818, 16689, 17249, 16920, 35364, 16822, 16840, 5570, 16820, 16913, 16924, 33591, 16880, 16847, 16901, 33596, 33956, 24858, 33602, 33607, 33831, 0 }, 	// CLASS 11 (Druid)
	};

	uint32 c = plr->getClass();
	for(uint32 i = 0; talentarray[c][i] != 0; ++i)
	{
		plr->addSpell(talentarray[c][i]);
	}
	return true;
}
*/
...
{	//Moved Unlearn command to the level 3 table. --Hemi
	...
	if(SpellId == 0)
	{	//Added link support to the unlearn command. --Hemi
